...
public abstract class Phase implements AutoCloseable {
	...
	protected Phase(String phaseName) {
		String loggedPhase = pins.Compiler.cmdLineArgValue("--logged-phase");
		if ((loggedPhase != null) && loggedPhase.matches(phaseName + "|all")) {
			// Prepare the name of the xml file.
			String xmlFileName = pins.Compiler.cmdLineArgValue("--xml");
			if (xmlFileName == null) {
				xmlFileName = pins.Compiler.cmdLineArgValue("--src-file-name").replaceFirst("\\.[^./]*$", "") + "."
						+ phaseName + ".xml";
			}

			// Prepare the name of the supporting xsl file.
			String xslDirName = pins.Compiler.cmdLineArgValue("--xsl");
			if (xslDirName == null) {
				xslDirName = "";
			}
